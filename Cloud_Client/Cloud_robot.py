...
class Cloud(object):
    ...
    def __init__(self, uri1='http://127.0.0.1:8270', timeout=None):
        ...
        urilist=uri1.split("http://")
        tagalist=urilist[1].split("/")
        uri="http://"+tagalist[0]
        self._uri = uri

        if len(tagalist)==2:
            addrtmp=tagalist[1]
            addrlist=addrtmp.split("@")
            if len(addrlist)==2:
                self.taga=addrlist[0]
                addr=addrlist[1]      
                client=XmlRpcRemoteClient(uri, timeout)
                self._ip=client.get_cloud_address(addr,self.taga)
                self._client = XmlRpcRemoteClient(self._ip, timeout)
            else:    
                self.taga=addrlist[0]
    ...
    def get_keyword_names(self, attempts=2):
        for i in range(attempts):
            time.sleep(i)
            try:
                return self._client.get_keyword_names_by_tag(self.taga)
    ...
    def get_keyword_arguments(self, name):
        try:
            return self._client.get_keyword_arguments_by_tag(name,self.taga)
        except TypeError:
            return ['*args']

    def get_keyword_documentation(self,name):
        try:
            return self._client.get_keyword_documentation(name,self.taga)
    ...
    def run_keyword(self, name, args, kwargs):
        ...
        result = RemoteResult(self._client.run_keyword(name,self.taga,args, kwargs))
...
class XmlRpcRemoteClient(object):
    ...
    def get_keyword_names_by_tag(self,tag=None):
        try:
            return self._server.get_keyword_names_by_tag(tag)
        except (socket.error, xmlrpclib.Error) as err:
            raise TypeError(err)

    def get_cloud_address(self,addr,taga):
        try:
            return self._server.get_cloud_address(addr,taga)
        except (socket.error, xmlrpclib.Error) as err:
            raise TypeError(err)        
    ...
    def get_keyword_documentation(self, name,tag):
        #print "gou"
        try:
            return self._server.get_keyword_documentation(name,tag)
    ...
    def get_keyword_arguments_by_tag(self, name,tag=None):
        try:
            return self._server.get_keyword_arguments_by_tag(name,tag)
        except xmlrpclib.Error:
            raise TypeError

    def get_keyword_documentation_by_tag(self, name,tag=None):
        #print "dao"
        try:
            return self._server.get_keyword_documentation_by_tag(name,tag)
        except xmlrpclib.Error:
            raise TypeError

    def run_keyword(self,name,tag,args, kwargs):
        run_keyword_args = [name,tag,args,kwargs] if kwargs else [name,tag,args]
        try:
    ...
    def run_keyword_bak(self, name, args, kwargs):
