...
class RobotCloudServer(SimpleXMLRPCServer):
    ...
    def __init__(self, library, host='127.0.0.1', port=8270, port_file=None,
        ...
        self._runliby=None
        #self._library=Selenium2Library()
    ...
    def _register_functions(self):
        self.register_function(self.get_keyword_names_by_tag)
        self.register_function(self.get_keyword_arguments_by_tag)
        #self.register_function(self.get_keyword_documentation_by_tag)
        self.register_function(self.get_library_keyword)
    ...
    def _announce_start(self, port_file=None):
        ...
        self._log('Robot Framework Cloud server at %s:%s starting.'
                  % (host, port))
        if port_file:
            pf = open(port_file, 'w')
            try:
    ...
    def get_library_keyword(self,liby):
        get_kw_names = getattr(liby, 'get_keyword_names', None) or \
                       getattr(liby, 'getKeywordNames', None)
        if self._is_function_or_method(get_kw_names):
            names = get_kw_names()
        else:
            names = [attr for attr in dir(liby) if attr[0] != '_' and
                     self._is_function_or_method(getattr(liby, attr))]
        #return names + ['stop_remote_server']
        return names   

    def get_keyword_names_by_tag(self,tag):
        tmpliby=self._library.get_library(tag)
        if type(self._runliby)==type(tmpliby):
            pass
        else:
            self._runliby=tmpliby
        names=self.get_library_keyword(self._runliby)
        #print names
        print "the action is importing: "+tag
        #return names + ['stop_remote_server']
        return names 
    ...
    def run_keyword(self,name,tag,args,kwargs=None):
        #liby=self._library.get_library(tag)
        args, kwargs = self._handle_binary_args(args, kwargs or {})
        result = {'status': 'FAIL'}
        self._intercept_std_streams()
        try:
            return_value = self._get_keyword_by_tag(name,tag)(*args, **kwargs)
        except:
    ...
    def run_keyword_bak(self, name, args, kwargs=None):
        args, kwargs = self._handle_binary_args(args, kwargs or {})
        result = {'status': 'FAIL'}
        self._intercept_std_streams()
        try:
    ...
    def get_keyword_arguments_by_tag(self,name,tag):
        kw = self._get_keyword_by_tag(name,tag)
        if not kw:
            return []
        return self._arguments_from_kw(kw)    
    ...
    def get_keyword_documentation(self,name,tag):
        tmpliby=self._library.get_library(tag)
        if type(self._runliby)==type(tmpliby):
            pass
        else:
            self._runliby=tmpliby
        if name == '__intro__':
            return inspect.getdoc(self._runliby) or ''
        if name == '__init__' and inspect.ismodule(self._runliby):
            return ''
        return inspect.getdoc(self._get_keyword_by_tag(name,tag)) or ''    
    ...
    def _get_keyword_by_bak(self,name,tag):
        liby=self._library.get_library(tag)
        if name == 'stop_remote_server':
            return self.stop_remote_server
        kw = getattr(liby, name, None)
        if not self._is_function_or_method(kw):
            return None
        return kw
